...
## 파일 쓰기
my_file = open("./myvenv/basic/chapter10/my-data.txt", "w", encoding = "utf8") # open()으로 열게 되면 객체 형태로 불러옴 # 절대 경로를 안 잡아주면 프로젝트 루트 디렉토리를 기준으로 함
my_file.write("Python 공부")
my_file.close()

## 파일 추가(이어 쓰기)
my_file = open("./myvenv/basic/chapter10/my-data.txt", "a", encoding = "utf8")
my_file.write("\nDjango 공부")
my_file.close()

## 파일 읽기
my_file = open("./myvenv/basic/chapter10/my-data.txt", "r", encoding = "utf8")
...
pickle_file = open("./myvenv/basic/chapter10/my-pickle.pickle", "wb") # wb: binary로 쓰기 # 확장자는 pickle, p, pic 가능
...
# pickled_file = open("./myvenv/basic/chapter10/my-pickle.pickle", "rb")
# pickled_data = pickle.load(pickled_file)
# print(pickled_data)
# pickle_file.close()

with open("./myvenv/basic/chapter10/my-pickle.pickle", "rb") as pickled_file: # with 구문을 사용하면 간결
